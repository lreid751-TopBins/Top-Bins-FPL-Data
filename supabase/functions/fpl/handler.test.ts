/**
 * Handler tests. No network access needed, but the shared-copy drift check
 * reads two directories on disk:
...
// rating.ts imports from ../_shared/, not public/js/ directly, because
// Supabase's deployed runtime doesn't include anything outside
// supabase/functions/ - a prior version that reached into public/js/
// passed every local/CI check and then crashed the whole function in
// production. _shared/ is a byte-for-byte copy that has to be re-made by
// hand after editing an original; this test is the tripwire that catches
// a forgotten copy before it ships, not deploy-time verification.
Deno.test("shared copies of the browser engine stay in sync with public/js", async () => {
  const files = ["api.js", "store.js", "planner.js", "projection.js", "teamRating.js"];
  for (const name of files) {
    const original = await Deno.readTextFile(new URL(`../../../public/js/${name}`, import.meta.url));
    const copy = await Deno.readTextFile(new URL(`../_shared/${name}`, import.meta.url));
    assert(
      copy === original,
      `supabase/functions/_shared/${name} is out of sync with public/js/${name} - run: cp public/js/${name} supabase/functions/_shared/${name}`
    );
  }
});
